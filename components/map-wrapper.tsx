...
const MapWrapper = (props) => {
    ...
    useEffect(()=>{
        console.log("Effect")
        !isRecenter && setCenter(props.location);
    },[props.location])

    const onIdle = (m: google.maps.Map) => {
        console.log("onIdle");
        setZoom(m.getZoom()!);
        const loc = m.getCenter()!.toJSON()
        if(loc.lat.toFixed(4) !== props.location.lat.toFixed(4) || loc.lng.toFixed(4) !== props.location.lng.toFixed(4)){
            setCenter(loc);
            setIsRecenter(true);
        }else{
        ...
        <Flex h="100%" w="100%" sx={{'& > div':{width:"100%", height:"100%", flexGrow: "1"}}}>
            <Wrapper apiKey={API_KEY} render={render} language="hi" region="IN">
                ...
                >
                    ...
                    {getUsersLocation().map((v)=>(<Marker key={v[0]} icon={svgMarker((v[1] as any).color)} label={{text:`${v[0][0].toUpperCase() + v[0].substr(1)}`,fontWeight:"700",fontSize:"14px"}} position={(v[1] as any).location} />))}
                    {getUsersLocation().map((v)=>(<Line key={v[0]} path={[props.location, (v[1] as any).location]} strokeColor={(v[1] as any).color} />))}
                </MapComponent>
            </Wrapper>
        </Flex>
